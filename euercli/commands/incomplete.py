...
def cmd_incomplete_list(args):
    ...
    if args.type in (None, "expense"):
        query = """
            SELECT e.id, e.date, e.vendor AS party, e.category_id, c.name AS category_name,
    ...
    if args.type in (None, "income"):
        query = """
            SELECT i.id, i.date, i.source AS party, i.category_id, c.name AS category_name,
    ...
    print("Fehlende Felder: category, receipt, vat, account (abhängig von Typ/Steuermodus).")
    conn.close()
